...
function animateRain() {
  ...
  ctx.fillStyle = rainColor === '#000000' && !randomSelected
    ...
    : `rgba(0, 0, 0, 0.${fontSize > fontSizes[2] ? '1' : '03'})`;

  ctx.fillRect(0, 0, canvas.width, canvas.height);

  // Set letter spacing by adjusting the spacing multiplier (0.7 for tighter spacing)
  const spacingMultiplier = 0.7;
  ...
  for (let i = 0; i < skeets.length; i++) {
    ...
    if (character) {
      writeCharacter(rainColor === '#000000' && !randomSelected ? "#000000" : "#ffffff",
        character,
        skeets[i].position * fontSize * spacingMultiplier,
        i * fontSize
      );
      ...
      if (skeets[i].position >= 3) {
        ...
        if (oldCharacter) {
          writeCharacter(
            characterColor,
            oldCharacter,
            (skeets[i].position - 2) * fontSize * spacingMultiplier,
            i * fontSize
          );
        }
      }
    ...
    if (skeets[i].position * fontSize * spacingMultiplier > canvas.width) {
      // color the last 2 characters on the grid otherwise they stay white
      writeCharacter(
        characterColor,
        character,
        (skeets[i].position - 1) * fontSize * spacingMultiplier,
        i * fontSize
      );
